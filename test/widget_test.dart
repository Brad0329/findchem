...
void main() {
  /// 저장본 없는 기기 + [bundle]을 번들로 쓰는 앱.
  FindChemApp appWith(Future<Dataset> bundle) =>
      FindChemApp(controller: DataController(store: MemoryUpdateStore(), loadBundled: () => bundle));

  testWidgets('번들 JSON을 실제 asset 경로로 읽는다(경로·형식이 틀리면 여기서 잡힌다)', (tester) async {
    // asset 읽기는 실제 I/O라 fake-async 밖에서 돌린다
    final loaded = await tester.runAsync(() => loadDataset(store: MemoryUpdateStore()));
    expect(loaded, isNotNull);
    expect(loaded!.origin, DataOrigin.bundled);
    expect(loaded.dataset.entries.length, 1657);
  });

  testWidgets('앱: 읽는 동안 진행 표시 → 검색 화면(헤더 FindChem, 검색창)', (tester) async {
    final completer = Completer<Dataset>();
    await tester.pumpWidget(appWith(completer.future));
  ...
  testWidgets('테마: Material 3, 아이콘 파랑 시드, 시스템 따라 라이트/다크(화면 방향 3·4)', (tester) async {
    await tester.pumpWidget(appWith(Completer<Dataset>().future));
